...
axis([-0.5 1.5 -0.15 0.35]);

%{
noturn_all = [];
turn_all = [];
for i = 1:length(turn)
    noturn_all = [noturn_all,noturn{i}];
    turn_all = [turn_all,turn{i}];
end
length(find(noturn_all>0))
length(find(turn_all>0))


change = [];
for i = 1:length(turn)
    if length(noturn{i}) >= 2 & length(turn{i}) >= 2
        change = [change,mean(turn{i})-mean(noturn{i})];
    end
end
