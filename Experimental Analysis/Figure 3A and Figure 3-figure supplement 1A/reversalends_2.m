...
for i = 1:trial
    ...
    time{i} = xlsread('time new.xlsx',i);
end
    ...
    for j = 1:size(time{i},2)
        %if isnan(time{i}(3,j)) ~= 0
            ...
            if isnan(time{i}(3,j)) == 1
                noturnN = noturnN+1;
                n = noturnN;
                plot_t = (time{i}(2,j)):(time{i}(2,j)+150);
            else
                turnN = turnN+1;
                n = turnN;
            ...
            % turn开始之后，turn中
            %if isnan(time{i}(3,j)) == 1     % for type-1 only
            %if isnan(time{i}(3,j)) ~= 1     % for type-2 only
            if 1                            % for both
                for t = plot_t
...
title('RIV GCaMP before and after reversal ends');
xlabel('t/s');
ylabel('dR/R0');
xlim([-3 3]);
%% heat map
...
for i = 1:trial
    for j = 1:size(time{i},2)
        if isnan(time{i}(3,j)) == 0
            ...
            hold on
            if j ~= size(time{i},2)
                endtime = min((time{i}(2,j)+150),(time{i}(1,j+1)-1));
            else
                endtime = (time{i}(2,j)+150);
            end
